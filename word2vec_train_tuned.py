		...
		for num in range(3):
			...
			while True:
				if new_acc <= old_acc:
					...
					n_equals = 0
				old_acc = new_acc
				embeddings, weights = train_by_name(train_x[num][0], train_y[num][0], bigram_dictionaries, filename, str(num+1), current_model, load) 
				if 'cosine' in current_model:
					if 'bigram' in current_model:
						new_acc = evaluate_word2vec_cosine(validate_x[num], validate_y[num], embeddings, weights, bigram_unused_dictionary, "results.csv", bigram_split=True)
					else:
					...
					if 'bigram' in current_model:
						new_acc = evaluate_word2vec_output(validate_x[num], validate_y[num], embeddings, weights, bigram_unused_dictionary, "results.csv", bigram_split=True)
					else:
						new_acc = evaluate_word2vec_output(validate_x[num], validate_y[num], embeddings, weights, bigram_unused_dictionary, "results.csv", bigram_split=False)
